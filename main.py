...
def eval_logs(event_path):
    ...
    _, _, vals = zip(*event_acc.Tensors('coverage'))
    coverage = [tf.make_ndarray(val) for val in vals]

    _, _, vals = zip(*event_acc.Tensors('energy_consumption'))
    ec = [tf.make_ndarray(val) for val in vals]

    _, _, vals = zip(*event_acc.Tensors('data_rate'))
    dr = [tf.make_ndarray(val) for val in vals]

    _, _, vals = zip(*event_acc.Tensors('delay'))
    delay = [tf.make_ndarray(val) for val in vals]
    
    _, _, vals = zip(*event_acc.Tensors('packet_delivery_rate'))
    pdr = [tf.make_ndarray(val) for val in vals]
    
    
    _, _, vals = zip(*event_acc.Tensors('loss'))
    loss = [tf.make_ndarray(val) for val in vals]

    print("Successful service coverage:", sum(coverage) / len(coverage))
    print("Average energy consumption:", sum(ec) / len(ec))
    print("Data rate:", sum(dr) / len(dr))
    print("Average delay:", sum(delay) / len(delay))
    print("Packet delivery rate:", sum(pdr) / len(pdr))
    print("Overhead:", sum(loss) / len(loss))
