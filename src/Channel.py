...
class ChannelParams:
    def __init__(self):
        self.base_station_snr = -25  # used a fixed value in dB 
        self.los_path_loss_exp = 1.27 # I set a fixed value for per 10 m
        self.nlos_path_loss_exp = 2.64 # I set a fixed value for per 10 m
        self.uav_altitude = 100.0  # in m
        self.base_station_coverage = 1000.0  # in m
        self.los_shadowing_variance = 2.0
        self.nlos_shadowing_variance = 5.0
        self.map_path = "res/ccu.png"
...
class Channel:
    ...
    def reset(self, area_size):
        self._norm_distance = np.sqrt(2) * 0.5 * area_size * self.params.base_station_coverage
        self.los_norm_factor = 10 ** (self.params.base_station / 10) / (
    ...
    def compute_rate(self, uav_pos, device_pos):
        dist = np.sqrt(
            ((device_pos[0] - uav_pos[0]) * self.params.base_station_coverage) ** 2 +
            ((device_pos[1] - uav_pos[1]) * self.params.base_station_coverage) ** 2 +
